...
imdsv1_check=$(curl -s -o /dev/null $metadata_endpoint; echo $?)
echo "IMDSv1 check result is = " ${imdsv1_check}
imdsv2_check=$(curl -s -o /dev/null -X PUT ${imdsv2_token_endpoint} -H "X-aws-ec2-metadata-token-ttl-seconds: 10"; echo $?)
echo "IMDSv2 check result is = " ${imdsv2_check}
